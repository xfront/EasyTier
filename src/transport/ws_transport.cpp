...
async_net::Task<bool> ws_transport::accept_connection() {
    ...
    // TODO: Perform proper WebSocket HTTP upgrade handshake
    // For now, create a websocket_connection directly on the raw socket
    // This will only work if the client also skips the HTTP upgrade
    auto peer = std::make_shared<ws_peer>();
    peer->sock = std::make_shared<async_net::tcp::socket>(std::move(sock));
    peer->conn = std::make_unique<async_net::http::ws::websocket_connection>(*peer->sock);
...
async_net::Task<bool> ws_transport::send(const uint8_t* data, size_t len,
    ...
    co_return co_await peer->conn->send_binary(data, len);
}
...
async_net::Task<std::optional<transport_recv_result>> ws_transport::receive() {
    ...
    while (running_) {
        ...
        for (auto& peer : peers_copy) {
            ...
            auto msg = co_await peer->conn->receive();
            if (!msg.empty()) {
                transport_recv_result result;
                result.data.assign(msg.begin(), msg.end());
